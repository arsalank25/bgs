...
try{
	...
	custumerIdNotANumber = true;
}

if(custId.equals(null) || custId.equals("")){    // Determine if invalid customer id was entered
	%>
	<h2> out.println("Invalid customer id. Go back to the previous page and try again."); </h2> 
	<% 
} else if (productList.size() == 0){ // Determine if there are products in the shopping cart
	%>
	<h2> out.println("Your shopping cart is empty!"); </h2>  
	<%
} else if (custumerIdNotANumber) {
	%>
	<h2> out.println("Invalid customer id. Go back to the previous page and try again."); </h2> 
	<% 
} else {
	try {
		...
		insert = "INSERT INTO Orders(customerId) VALUES (?)";		
