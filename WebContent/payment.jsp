...
<body>
	<div style="background-color: orange;">
		<h2>Choose existing payment method</h2>
	</div>


	<%
		//Are we removing a payment?
		String removePayment = request.getParameter("remove");
		...
		if (pname != null) {
			...
			try {
				...
				if (removePayment != null) {
					sql = "DELETE FROM Payment WHERE paymentName = ?";
					con = DriverManager.getConnection(url, uid, pw);
					PreparedStatement prep = con.prepareStatement(sql);
					prep.setString(1, removePayment);					
					...
					while (rs.next()) {
						...
						out.print("<a style =\"display: inline;\" href=\"pay.jsp\">" + paymentName + "</a>      ");
						out.print("<a style =\"display: inline;\" href=\"payment.jsp?remove=" + paymentName
								+ "\">(Remove)</a><br>");
					}
	...
	<div style="background-color: orange;">
		<h2>Add new Payment Information</h2>
	</div>
	<form name="paymentForm" method="POST" action="payment.jsp"
		autocomplete="on">
		...
		<div class="container">
			...
			<label><b>Payment Name</b></label> <input type="text"
				placeholder="Enter Payment Name" name="pname" required> <label><b>First
					Name</b></label> <input type="text" placeholder="Enter First Name" name="fname"
				required> <label><b>Last Name</b></label> <input type="text"
				placeholder="Enter Last Name" name="lname" required> <label><b>Street</b></label><br>
			<input type="text" placeholder="Enter Street" name="street" required><br>

			<label><b>City</b></label> <input type="text"
				placeholder="Enter City" name="city" required> <label><b>Province</b></label>
			<input type="text" placeholder="Enter Province" name="province"
				required> <label><b>Postal Code</b></label> <input
				type="text" placeholder="Enter Postal Code" name="postalcode"
				required> <br> <br> <img src="images/cc.gif">
			<br> <br> <label><b>Card Number</b></label> <input
				type="text" placeholder="Enter City" name="ccnum" required
				autocomplete="shipping locality"> <label><b>Card
					Security Code</b></label> <input type="text" placeholder="CardSecurity Code"
				name="sec" required> <label><b>Expiry
					date(mm/yy)</b></label> <input type="text" placeholder="mm/yy" name="exp"
				required>
